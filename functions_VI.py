...
class Dataset_CRNN_2(data.Dataset):
    "Characterizes a dataset for PyTorch"
    def __init__(self, data_path, folders, labels, frames, transform=None):
        "Initialization"
        self.data_path = data_path
...
class Dataset_CRNN_3(data.Dataset):
    "Characterizes a dataset for PyTorch"
    def __init__(self, data_path, folders, labels, frames, transform=None):
        "Initialization"
        self.data_path = data_path
...
class EncoderCNN(nn.Module):
    def __init__(selfResCNNEncoder, img_x=90, img_y=120, fc_hidden1=512, fc_hidden2=512, drop_p=0.3, CNN_embed_dim=300):
...
class ResCNNEncoder(nn.Module):
    def __init__(self, fc_hidden1=512, fc_hidden2=512, drop_p=0.3, CNN_embed_dim=300):
        ...
        self.fc_hidden1, self.fc_hidden2 = fc_hidden1, fc_hidden2
        self.drop_p = drop_p
        features = ['4.0.conv1', "4.1.conv1"]

        resnet = models.resnet152(pretrained=True)
        
        modules = list(resnet.children())[:-1]      # delete the last fc layer.
        self.resnet = nn.Sequential(*modules)
        #nodes, _ = get_graph_node_names(self.resnet)
        #print(nodes)
        self.feature_extractor = create_feature_extractor(self.resnet, return_nodes=features)
    ...
    def forward(self, x_3d):
        ...
        # swap time and sample dim such that (sample dim, time dim, CNN latent dim)
        cnn_embed_seq = torch.stack(cnn_embed_seq, dim=0).transpose_(0, 1)
        aa = self.feature_extractor
        # cnn_embed_seq: shape=(batch, time_step, input_size)

        return cnn_embed_seq,aa
...
class DecoderRNN(nn.Module):
    ...
    def forward(self, x_RNN):
        ...
        x_1 = F.relu(x)
        x_2 = F.dropout(x_1, p=self.drop_p, training=self.training)
        x_3 = self.fc2(x_2)
        """ 返回最后一个隐含层和输出层""" 
